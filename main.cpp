...
void apply_mean_filter(const char *input_image, int kernel_size)
{
    ...
    std::string output_filename = "mean_filtered_" + remove_extension(input_image) + "_" + std::to_string(kernel_size) + "x" + std::to_string(kernel_size) + ".png";
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        {
            if (argc < 4)
                throw std::invalid_argument("Usage: clearvision mean <img> <kernel_size>");
            apply_mean_filter(argv[2], std::stoi(argv[3]));
        }
        else if (operation == "gauss")
        {
            if (argc < 5)
                throw std::invalid_argument("Usage: clearvision gauss <img> <kernel_size> <sigma>");
            apply_gaussian_smoothing(argv[2], std::stoi(argv[3]), std::stof(argv[4]));
        }
        else if (operation == "unsharp")
        {
            if (argc < 5)
                throw std::invalid_argument("Usage: clearvision unsharp <img> <kernel_size> <amount>");
            apply_unsharp_mask(argv[2], std::stoi(argv[3]), std::stof(argv[4]));
        }
        else if (operation == "add")
        {
            if (argc < 4)
                throw std::invalid_argument("Usage: clearvision add <img1> <img2>");
            add_images(argv[2], argv[3]);
        }
        else if (operation == "sub")
        {
            if (argc < 4)
                throw std::invalid_argument("Usage: clearvision sub <img1> <img2>");
            subtract_images(argv[2], argv[3]);
        }
        else if (operation == "equals")
        {
            if (argc < 4)
                throw std::invalid_argument("Usage: clearvision equals <img1> <img2>");
            compare_images(argv[2], argv[3]);
        }
        else if (operation == "disguise")
        {
            if (argc < 3)
                throw std::invalid_argument("Usage: clearvision disguise <img>");
            disguise_image(argv[2]);
        }
        else if (operation == "reveal")
        {
            if (argc < 3)
                throw std::invalid_argument("Usage: clearvision reveal <dat>");
            reveal_image(argv[2]);
        }
        else if (operation == "enc")
        {
            if (argc < 4)
                throw std::invalid_argument("Usage: clearvision enc <img> <message>");
            encrypt_image(argv[2], argv[3]);
        }
        else if (operation == "dec")
        {
            if (argc < 4)
                throw std::invalid_argument("Usage: clearvision dec <img> <msg_len>");
